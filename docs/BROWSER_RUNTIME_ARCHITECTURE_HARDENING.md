...
- **Stable product surface - preserved.** The public architecture remains one `moondesk` executable, one lazy host-shared browser session, and only MCP `browser_command` + `view_page`. `tools/call` also accepts only the connector-defined expanded browser operation allowlist as a compatibility ingress, normalizes pinned commands through the same v1.7 contract, and routes them into the shared runtime without advertising a second public tool surface. Connector-only `fill_form` and `wait_for` are MoonDesk-owned composites implemented on top of pinned `fill`/`take_snapshot` operations under the same serialization, deadline, read-only, and workspace-boundary rules. The host-shared browser remains an explicit trust-domain decision.
