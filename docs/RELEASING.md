...
6. **Five-platform matrix** — Linux x64, Linux arm64, macOS Intel, macOS arm64, and Windows x64 compile the exact candidate. Every binary runs a ClippyMoon help/export smoke test before becoming an artifact; the Windows candidate also reruns the canonical-config/migration/reset regressions so a release cannot publish if the affected-device persistence behavior regresses.
