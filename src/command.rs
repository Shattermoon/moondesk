...
fn shell_words(segment: &str) -> Vec<ShellWord> {
    ...
    for (idx, ch) in segment.char_indices() {
        ...
        if ch == '\\' && !in_single {
            ...
            if cfg!(windows) {
                // MoonDesk uses PowerShell as its host shell on Windows, where
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn shell_words_preserve_windows_path_separators() {
        let words = shell_words(r#"C:\Windows\System32\cmd.exe "C:\Program Files\Tool\tool.exe""#);
        assert_eq!(words[0].text, r"C:\Windows\System32\cmd.exe");
        assert_eq!(words[1].text, r"C:\Program Files\Tool\tool.exe");
    }
